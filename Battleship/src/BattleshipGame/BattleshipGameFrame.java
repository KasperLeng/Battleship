...
public class BattleshipGameFrame implements ActionListener {
	...
	public JLabel pShipsSunk;

	public JLabel cShots;
	public JLabel cHits;
	public JLabel cMisses;
	public JLabel cShipsSunk;
	...
	/*** CONSTRUCTOR ***/
	public BattleshipGameFrame(boolean isHard) throws IOException {
        if(isHard == false){
            isComplex = false;
        } else{
            isComplex = true;
		...
		pShipsSunk = new JLabel("Ships Sunk: " + String.valueOf(pTotalShipsSunk));
		pShipsSunk.setFont(new Font("Monospaced", Font.BOLD, 15));
		pShipsSunk.setForeground(new Color(57, 255, 20));
		pShipsSunk.setBounds(25, 171, 225, 43);
		playerInfo.add(pShipsSunk);
		...
		cShipsSunk = new JLabel("Ships Sunk: " + String.valueOf(cTotalShipsSunk));
		cShipsSunk.setFont(new Font("Monospaced", Font.BOLD, 15));
		cShipsSunk.setForeground(new Color(57, 255, 20));
		cShipsSunk.setBounds(25, 171, 225, 43);
		computerInfo.add(cShipsSunk);
		...
		firstTurn = 1;//(int) (Math.random() * 2); // 0 = player goes first, 1 = computer goes first
 ...
 // Method plays Explosion sound effect when ship is hit.
	...
	public void actionPerformed(ActionEvent e) {
				...
				for (int c = 0; c < computerButtons[r].length; c++) {
					...
					if (e.getSource() == computerButtons[r][c]) {
						...
						if (playerMoves.isMoveValid(r, c, computerGrid)) {
							...
							// update player's info given a hit, miss, or sink
							if (pHitValue == -1) {
								pTotalMisses++;
								...
								pTotalShipsSunk++;
								pShipsSunk.setText("Ships Sunk: " + String.valueOf(pTotalShipsSunk));
							}
							// update total shots
							pTotalShots++;
							pShots.setText("Total Shots: " + String.valueOf(pTotalShots));
							// check if player has sunk all ships - placed music
							...
							// update the computer's info given a hit, miss, or sink
							if (cHitValue == -1) {
								cTotalMisses++;
								cMisses.setText("Total Misses: " + String.valueOf(cTotalMisses));
							} else if (cHitValue == 1) {
								...
								cTotalShipsSunk++;
								cShipsSunk.setText("Ships Sunk: " + String.valueOf(cTotalShipsSunk));
							}
							// update total shots
							cTotalShots++;
							cShots.setText("Total Shots: " + String.valueOf(cTotalShots));
		...
		if (e.getSource() == playAgain) {
			// instantiate the intro frame to start the game all over again
