...
public class AI {
	 ...
	 */
	public int simpleAI(int playerGrid[][], JButton playerButtons[][], int shipLengths[], int numShipHitsP[], int playerShipPos[][], JLabel shipNames[]) {
	 ...
	 * @return value returned from making a move (hit, miss, or sunk)
	 */
	public int complexAI(int playerGrid[][], JButton playerButtons[][], int shipLengths[], int numShipHitsP[], int playerShipPos[][], JLabel shipNames[]) {
		...
		if (shipIndex < 0) {
			...
			// Random coordinate if the diagonals of the coordinate are invalid
				...
				randomCoordinate(playerGrid);
			// Goes diagonally if there is valid coordinates
			else 
		...
		// If there is ship partially hit only once
			...
			// regenerate direction and coordinates if current coordinates are out of bounds or not valid
			do {
			...
			if (hitValue == 1) {
				// if there's another hit, the direction of the ship is known --> store direction
		...
		// If more than two points of a ship is discovered
			...
			if(!isCoordinateValid(tempCoordinateR, tempCoordinateC, playerGrid)){
				// switch to opposite direction (i.e. originally left, search right or originally down search up)
				tempDirectionR *= -1;
				tempDirectionC *= -1;
	...
	/**
	 * This method will mark the move on GUI frame and update the value in the array.
	 ...
	 * @return
	 */
	public int makeMove(int playerGrid[][], JButton playerButtons[][], int shipLengths[], int numShipHitsP[], int playerShipPos[][], JLabel shipNames[]) {
	...
	/**
	 * This method will check if any ship has been hit one or more times, but not fully sunk
	 * @param shipLengths
	 * @param numShipHitsP
	 * @return the index of a ship that has been partially hit, -1 if no ship has been partially hit
	 */
